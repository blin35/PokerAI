...
if __name__ == "__main__":
    ...
    me = 'giguerefan36'
    tablename = 'HH20190404 Sarmiento'
    gamename = tablename + ' - $0.01-$0.02 - USD No Limit Hold\'em'

    f = open('HandHistoryArchive/' + gamename + '.txt')
    lines = f.readlines()
    i = 0
    ...
    #ID Links DB
    IDdb = load_id_links()
    lastrow_hand = IDdb.iloc[[-1]]
    colnames_IDdb = list(IDdb.columns.values)
    lasthandid = lastrow_hand["HandID"].values[0]

    #Preflop DB
    preflopdb = load_preflop_database()
    lastrow_preflop = preflopdb.iloc[[-1]]
    colnames_preflop = list(preflopdb.columns.values)
    print(lastrow_preflop)
    lastpreflopid = lastrow_preflop["PreflopID"].values[0]

    #Postflop DB
    postflopdb = load_postflop_database()
    lastrow_postflop = postflopdb.iloc[[-1]]
    colnames_postflop = list(postflopdb.columns.values)
    print(lastrow_postflop)
    lastpostflopid = lastrow_postflop["PostflopID"].values[0]

    currid = [lasthandid + 1, lastpreflopid + 1, lastpostflopid + 1]
